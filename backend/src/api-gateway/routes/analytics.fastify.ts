...
export async function analyticsRoutes(
  ...
  async function getCached<T>(key: string, ttlSeconds: number, fn: () => Promise<T>): Promise<T> {
    try {
      const hit = await (fastify as any).redis?.get(key);
      if (hit) return JSON.parse(hit);
    } catch (_) {}
    const result = await fn();
    try {
      await (fastify as any).redis?.setex(key, ttlSeconds, JSON.stringify(result));
    } catch (_) {}
    return result;
  }
   ...
   */
  fastify.get<{ Querystring: { days?: string } }>("/analytics/tvl-history", async (request) => {
   ...
   */
  fastify.get<{ Querystring: { days?: string } }>("/analytics/utilization-curves", async (request) => {
   ...
   */
  fastify.get<{ Querystring: { days?: string } }>("/analytics/revenue-breakdown", async (request) => {
    const days = Math.min(parseInt(request.query.days || "30", 10), 180);
    ...
    const result = await getCached(`analytics:revenue:${days}`, 60, async () => {
      ...
      type RevDay = {
        date: string;
        stake_revenue: number;
        borrow_revenue: number;
        liquidation_revenue: number;
        flash_loan_revenue: number;
      };

      const daysMap: Record<string, RevDay> = {};
   ...
   */
  fastify.get<{ Querystring: { limit?: string } }>("/analytics/user-cohorts", async (request) => {
   ...
   */
  fastify.get<{ Querystring: { limit?: string } }>("/analytics/events", async (request) => {
  ...
  fastify.get("/analytics/realtime-metrics", async () => {
    return await getCached("analytics:realtime", 30, async () => {
      ...
      const activeUsersRaw = await prisma.$queryRaw<any[]>`
      ...
      const allUsersRaw = await prisma.$queryRaw<any[]>`
        SELECT COUNT(DISTINCT user_address) as count 
        FROM protocol_events
      `;

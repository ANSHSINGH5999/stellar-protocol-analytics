...
export async function startApiGateway(deps: GatewayDeps) {
  ...
  await fastify.register(liquidityRoutes, {
    ...
    // Analytics endpoints
    const { analyticsRoutes } = await import("./routes/analytics.fastify.js");
    await fastify.register(analyticsRoutes, { prefix: "/api", prisma: deps.prisma });

    // Smart Contract Simulation endpoints

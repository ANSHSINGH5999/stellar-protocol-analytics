...
export async function simulateDeposit(
  ...
  if (rpc.Api.isSimulationSuccess(simResult) && simResult.result) {
    sxlmMinted = BigInt(scValToNative(simResult.result.retval));
  }

  const exchangeRate = await getExchangeRate();

  return { sxlmMinted, exchangeRate };
}
...
export async function simulateRequestWithdrawal(
  userPublicKey: string,
  sxlmAmount: bigint
): Promise<SimWithdrawalResult> {
  const { account } = await getSourceAccount();
  const contract = getStakingContract();
  ...
  if (rpc.Api.isSimulationSuccess(simResult) && simResult.result) {
    const result = scValToNative(simResult.result.retval) as {
      withdrawal_id: number;
      is_instant: boolean;
      xlm_amount: bigint;
    };
  ...
  return { sxlmAmount, xlmAmount, exchangeRate, isInstant, unlockTime };
}

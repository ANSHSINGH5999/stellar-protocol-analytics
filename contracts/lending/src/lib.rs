...
impl LendingContract {
    ...
    /// Optimized Flash Loan for XLM liquidity.
    /// User receives XLM, executes custom logic, and must repay amount + 0.05% fee.
    pub fn flash_loan(env: Env, receiver: Address, amount: i128) {
        ...
        let native = read_native_token(&env);
        let native_client = token::Client::new(&env, &native);
        let pool_balance_before = native_client.balance(&env.current_contract_address());
        assert!(pool_balance_before >= amount, "insufficient liquidity");

        // 1. Transfer XLM to receiver
        native_client.transfer(&env.current_contract_address(), &receiver, &amount);

        // 2. Compute fee (5 bps = 0.05%)
        let fee = amount * 5 / 10000;
        let total_due = amount + fee;

        // 3. Trigger receiver auth/call logic (simplified for Soroban SAC transfer)
        // In a real protocol, we'd use a callback interface. 
        // Here we enforce balance check after the call.
        
        // 4. Enforce repayment (receiver must have transferred back or we just pull it)
        native_client.transfer(&receiver, &env.current_contract_address(), &total_due);
